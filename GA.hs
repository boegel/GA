{--
 - Haskell module for working with genetic algoritms
 -
...
-- take and drop elements in a single pass
...
-- * archiveSize: size of archive (best entities so far)
-- * maxGenerations: maximum number of generations to evolve
-- * crossoverRate: fraction of entities generated by crossover (tip: >= 0.80)
-- * mutationRate: fraction of entities generated by mutation (tip: <= 0.20)
-- * crossoverParam: parameter for crossover (semantics depend on actual crossover operator)
-- * mutationParam: parameter for mutation (semantics depend on actual mutation operator)
-- * withCheckpointing: enable/disable built-in checkpointing mechanism
...
-- typeclass for entities that represent a candidate solution
-- three parameters:
-- type a: data structure representing an entity
-- type b: data used to score an entity, e.g. a list of numbers
-- type c: some kind of pool used to generate random entities, e.g. a Hoogle database
...
-- typeclass for pretty print an entity instead of just using the default show implementation
...
evolutionStep :: (Entity a b c) => c -> b -> (Int,Int,Int) -> (Float,Float) -> ([ScoredEntity a],[ScoredEntity a]) -> (Int,Int) -> ([ScoredEntity a],[ScoredEntity a])
...
restoreFromCheckpoint :: (Entity a b c) => GAConfig -> [(Int,Int)] -> IO (Maybe (Int,([ScoredEntity a],[ScoredEntity a])))
...
checkpointGen :: (Entity a b c) => GAConfig -> Int -> Int -> ([ScoredEntity a],[ScoredEntity a]) -> IO()
...
evolution :: (Entity a b c) => GAConfig -> ([ScoredEntity a],[ScoredEntity a])
                            -> (([ScoredEntity a],[ScoredEntity a]) -> (Int,Int) -> ([ScoredEntity a],[ScoredEntity a]))
                            -> [(Int,Int)] -> IO ([ScoredEntity a],[ScoredEntity a])
...
evolve g cfg src dataset = do
                ...
                let ps = popSize cfg
                    ...
                    -- do the evolution
                    --(resPop,resArchive) = foldl (evolutionStep dataset (cCnt,mCnt,aSize) (crossPar,mutPar)) (zip (repeat Nothing) pop,[]) $ zip [0..] seeds
                restored <- if checkpointing
                               then restoreFromCheckpoint cfg (reverse genSeeds) :: (Entity a b c) => IO (Maybe (Int,([ScoredEntity a],[ScoredEntity a])))
                               else return Nothing
