...
evolutionStep :: (Entity e s d p m) => p -- ^ pool for crossover/mutation
                                  ...
                                  -> m (Universe e, -- ^ renewed universe
                                        Generation e s) -- ^ next generation
evolutionStep pool
              dataset
              (cn,mn,an)
              (crossPar,mutPar)
...
evolve g cfg pool dataset = do
    -- initialize
    (pop, cCnt, mCnt, aSize, 
     crossPar, mutPar, genSeeds) <- if not (withCheckpointing cfg)
...
-- |Do the evolution (support checkpointing). 
--
